...
std::string parseSql(std::string sentence, ExtendibleHash<RegistroNBA>& hashTable)
{
    for (char& i : sentence)
        i = std::toupper(i);
    std::istringstream sentence_stream(sentence);
    ...
    else if (currentWord == "SELECT")
    {
        ...
        if (currentWord == "NBA_AVL")
        {
            ...
            else if (currentWord == "EQUALS")
            {
                ...
                long keyl = atol(key.c_str());
                auto result = hashTable.search(keyl);
    ...
    else if (currentWord == "DELETE")
    {
        ...
        if (currentWord == "NBA_AVL")
        {
            ...
            sentence_stream >> key;
            long keyl = atol(key.c_str());
            bool worked = hashTable.remove(keyl);
            if (worked)
                return "Deleted \n";
            else
            {
